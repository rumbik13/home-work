// Задача 1.
// Напишите условие, которое будет проверять чётное ли число,
...
if (number / 2) {
  console.log("Это чётное");
} else {
  console.log("Это нечётное");
}
...
const role = prompt("Введите имя пользователя", "");
const password = prompt("Введите пароль", 0);

if (role === ADMIN || (USER && password === USER_PASSWORD)) {
...
const weight = prompt("Вес посылки?(в килограммах)", 1);
const salary = prompt(
  "Тип доставки? (Стандарт, Экспресс, Премиум)",
  "Стандарт",
);
let salary_type = "Стандарт" || "Экспресс" || "Премиум";
let coff;
let price;
...
// if (salary_type == "Стандарт") {
//   coff = 1;
// } else if (salary_type == "Экспресс") {
//   coff = 1.5;
// } else if (salary_type == "Премиум") {
...
if (weight > 0 && salary == salary_type) {
  alert(`Итоговая стоимость доставки: "${price * coff}$" `);
}
